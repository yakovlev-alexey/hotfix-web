...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order, details, setDetails }) => {

  const [ faster, setFaster ] = useState(details.time == null || details.faster);
  const [ time, setTime ] = useState(details.faster ? '' : details.time || '');
  const [ selfService, setSelfService ] = useState(details.selfService || false);
  const area = foodAreas.filter(area => area.id === areaId)[0];
  const item = area.items.filter(item => item.id === itemId)[0];

  useEffect(() => {
    setDetails({faster, time, selfService})
  }, [])
  ...
  return (
    <div className="Place">
        ...
        <div className="Place__choice-item">
          ...
          <input
            ...
            onChange={event => {
              ...
              if (!/^\d{2}:$/.test(time) 
              && (/^[0-1]\d$/.test(value) || /^2[0-3]$/.test(value))) {
                setTime(value + ":");
              } else if (/^[0-2]?$/.test(value) 
                || /^[0-1]\d$/.test(value) 
                || /^2[0-3]$/.test(value)
                || /^\d{2}:$/.test(value)
                || /^\d{2}:[0-5]$/.test(value)
                || /^\d{2}:[0-5]\d$/.test(value)) {
                setTime(value);
              }
            }}
            onBlur={() => {
      ...
      <footer className="Place__footer">
        <Link to={`/order/${area.id}/${item.id}`} className={price > 0 && (faster || /^\d{2}:\d{2}$/.test(time)) ? "Place__order" : "Place__order Place__order-disabled"}>
          Оплатить {price}
        </Link>
      </footer>
    </div>
